...
echo "RDS Instance Sizes per region" | tee gov_db_instance_sizes.txt
for engine in "${engines[@]}"; do
    echo "Engine: $engine" | tee -a gov_db_instance_sizes.txt
    for region in "${regions[@]}"; do
        version=`aws rds describe-db-engine-versions --engine $engine | jq '(.[] | reverse)[0]' | jq .EngineVersion | tr -d '"'`
        echo "Region: $region" | tee -a gov_db_instance_sizes.txt
        echo "--------------------------------------" | tee -a gov_db_instance_sizes.txt
        aws rds describe-orderable-db-instance-options --engine $engine --engine-version $version \
            ...
            --output text --region $region | tee -a gov_db_instance_sizes.txt
        echo "--------------------------------------" | tee -a gov_db_instance_sizes.txt
    done
done